...
int main(int argc, char **argv) {
  ...
  VectorXd eigs, deigs_dc11, deigs_da, deigs_dc44, _;
  ...
  for(int i = 0; i < nevs; i++) {
    if(std::abs(eigs(i) - ref_eigs[i]) > etol) {
      passed = false;
    }
  }
  ...
  for(int i = 0; i < nevs; i++) {
    if(std::abs(deigs_dc11(i) - ref_deigs_dc11[i]) > etol) {
      passed = false;
    }
  }
  ...
  VectorXd eigst_c11, eigst_a, eigst_c44;
  double delta = 0.00001;
  
  mechanics(c11 + delta, anisotropic, c44, // Params
            dp, pv, nevs, // Ref data
            eigst_c11, _, _, _); // Output

  mechanics(c11, anisotropic + delta, c44, // Params
            dp, pv, nevs, // Ref data
            eigst_a, _, _, _); // Output

  mechanics(c11, anisotropic, c44 + delta, // Params
            dp, pv, nevs, // Ref data
            eigst_c44, _, _, _); // Output

  for(int i = 0; i < nevs; i++) {
    double approxd = (eigst_c11(i) - eigs(i)) / delta;
    //printf("c11 %f %f\n", approxd, (*deigs_dc11)(i));
    if(std::abs(approxd - deigs_dc11(i)) > etol) {
      passed = false;
    }

    approxd = (eigst_a(i) - eigs(i)) / delta;
    //printf("a   %f %f\n", approxd, (*deigs_da)(i));
    if(std::abs(approxd - deigs_da(i)) > etol) {
      passed = false;
    }

    approxd = (eigst_c44(i) - eigs(i)) / delta;
    //printf("c44 %f %f\n", approxd, (*deigs_dc44)(i));
    if(std::abs(approxd - deigs_dc44(i)) > etol) {
      passed = false;
    }
  }  
